...
const REEM = path.join(ROOT, "reem");
...
function readCSV(file) {
  ...
  if (!fs.existsSync(file))
    throw new Error("Intelligence file not found: " + path.basename(file));
...
function reemCSV(name) {

  return path.join(REEM, name);
}
...
// ============================================================
// REEM — SUPPLY CHAIN INTELLIGENCE
// Read-only integration of generated intelligence outputs
// ============================================================

if (
  req.method === "GET" &&
  url.pathname === "/api/intelligence/summary"
) {
  ...
  try {
    ...
    const rows =
      readCSV(
        reemCSV("v13_management_summary.csv")
      );
    ...
    return json(
      res,
      {
        error: "Supply intelligence unavailable",
        details: err.message
      },
      503
    );
...
if (
  req.method === "GET" &&
  url.pathname === "/api/intelligence/drugs"
) {
  ...
  try {
    ...
    return json(
      res,
      readCSV(
        reemCSV("v13_drug_intelligence.csv")
      )
    );
    ...
    return json(
      res,
      {
        error: "Drug intelligence unavailable",
        details: err.message
      },
      503
    );
...
if (
  req.method === "GET" &&
  url.pathname === "/api/intelligence/suppliers"
) {
  ...
  try {
    ...
    return json(
      res,
      readCSV(
        reemCSV("v13_supplier_intelligence.csv")
      )
    );
    ...
    return json(
      res,
      {
        error: "Supplier intelligence unavailable",
        details: err.message
      },
      503
    );
...
if (
  req.method === "GET" &&
  url.pathname === "/api/intelligence/readiness"
) {
  ...
  try {
    ...
    return json(
      res,
      readCSV(
        reemCSV("v13_model_readiness.csv")
      )
    );
    ...
    return json(
      res,
      {
        error: "Model readiness unavailable",
        details: err.message
      },
      503
    );
...
if (
  req.method === "GET" &&
  url.pathname === "/api/intelligence/unmatched"
) {
  ...
  try {
    ...
    return json(
      res,
      readCSV(
        reemCSV("v13_unmatched_sales.csv")
      )
    );
    ...
    return json(
      res,
      {
        error: "Data-quality review queue unavailable",
        details: err.message
      },
      503
    );
